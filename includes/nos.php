...
// congiguración de las señales
    ...
    if (isset($argv[2])) {
        ...
        while (true) {
            ...
            {
                if (array_key_exists('audioDelay', $rChannelData))
                    $audioDelay=$rChannelData['audioDelay'];
                if (array_key_exists('encodeAudio', $rChannelData))
                    $encodeAudio=$rChannelData['encodeAudio'];
                ...
                while (true) {
                    ...
                    if ($rSegments && strlen($rSegments['video'])) {
                        ...
                        if ($rKeys) {
                            ...
                            $rCompleted = $segmentsMap->processSegments($rKeys, $rSegments, $rVideoDir . '/' . $rChannel, 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.63 Safari/537.36', $rScript,$audioDelay,$encodeAudio);
